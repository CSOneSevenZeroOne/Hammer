<template>
    ...
    <div class="content">
        <div  class="con-four">
            <ul>
                <li v-for="k in arr">
                    <div _ngcontent-c6="" class="checkbox-container"> <!----><span @click="checkbox" _ngcontent-c8="" class="checkbox-on m-blue-checkbox-new gou"> </span> <!----> </div>
                    <a href="#"><img :src="k.imgsrc" alt=""></a>
                    <div class="four-js"><h4 v-text="k.name"></h4>
                        <p class="jieshao">漂亮得不像实力派</p>
                        <p class="pie" v-show="!bool"><span>¥</span><a class="price" v-text="k.newprice.split('￥')[1].replace(',','')"></a><span _ngcontent-c6="" class="sum"><i>x</i> <a v-text="k.num"></a></span></p>
...
//			this.repeat()
			...
			checkbox:function (eve) {
				if($(eve.target).hasClass("gou")){//未选中时点击,切换成选中打勾,加入计算
					$(eve.target).removeClass("gou")
					$(eve.target).parents("li").addClass("list")
					count()
				}else {//选中时点击,切换成未选中状态,取消计算
					$(eve.target).addClass("gou")
					$(eve.target).parents("li").removeClass("list")
					count()
				}
				panduan()
				function panduan() {
				...
				function count() {
					...
					for (var i = 0; i < $(".list").length; i++) {
						allsum = allsum + parseInt($(".list").eq(i).find(".sum a").text())
						allpic = allpic + parseInt($(".list").eq(i).find(".sum a").text()) * parseFloat($(".list").eq(i).find(".price").text())
						console.log(parseFloat($(".list").eq(i).find(".price").text()));
					}
					$(".allsum").text(allsum)
					$(".allpic").text(allpic + ".00")
				}
		...
		mounted() {
			...
			if (!tel && typeof(tel)!="undefined" && tel!=0){
				if (!cart && typeof(cart)!="undefined" && cart!=0){
					return this.$store.state.boolcart
				}else {
					herf.arr=JSON.parse(cart)
					this.$store.state.num=herf.arr.length
					return this.$store.state.boolcart=true
				}
			}else {
				$.ajax({
					url:"http://localhost:8888/cart/watch",
					type : "POST",
					data : {
						tel
					}
